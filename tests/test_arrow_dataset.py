...
from datasets.features.features import MultiLabel
...
class BaseDatasetTest(TestCase):
    ...
    def test_multi_class_encode_column(self, in_memory):
        data = {
            'test_col': [['a', 'b'], ['b'], ['b', 'c'], ['a', 'd']]
        }

        with tempfile.TemporaryDirectory() as tmp_dir:
            dataset = Dataset.from_dict(data)

            res = dataset.class_encode_column('test_col')

            dataset_path = f'{tmp_dir}/test_dataset'
            res.save_to_disk(dataset_path)
            new_dataset = Dataset.load_from_disk(dataset_path)

            self.assertEqual(res.features['test_col'].feature.num_classes, new_dataset.features['test_col'].feature.num_classes)
